...
void test('migrates every private record, persists ciphertext only, unlocks and locks', async () => {
  ...
  const vault = await EncryptedVault.open(storage, password, true);
  ...
  assert.ok(!storage.getItem(VAULT_KEY)!.includes(password));
  const reopened = await EncryptedVault.open(storage, password, false);
...
void test('wrong password and tampering cannot unlock or modify ciphertext', async () => {
  const storage = store();
  await EncryptedVault.open(storage, password, true);
  const before = storage.getItem(VAULT_KEY)!;
  await assert.rejects(EncryptedVault.open(storage, 'wrong password', false));
  assert.equal(storage.getItem(VAULT_KEY), before);
  ...
  await assert.rejects(EncryptedVault.open(storage, password, false));
});
void test('failed migration preserves original plaintext; short passwords never create a vault', async () => {
  const storage = store();
  storage.setItem(PRIVATE_KEYS[0], 'original');
  await assert.rejects(EncryptedVault.open(storage, 'short', true));
  assert.equal(storage.getItem(VAULT_KEY), null);
  const full = { ...storage, setItem: () => { throw new Error('quota'); } };
  await assert.rejects(EncryptedVault.open(full, password, true));
...
void test('serializes updates, uses fresh IVs, rejects stale sessions and rolls back failed writes', async () => {
  const storage = store();
  const vault = await EncryptedVault.open(storage, password, true);
  const stale = await EncryptedVault.open(storage, password, false);
  ...
  assert.equal((await EncryptedVault.open(storage, password, false)).getItem(PRIVATE_KEYS[0]), 'retry');
});
void test('interrupted cleanup can be resumed after unlock; existing vault cannot be overwritten by setup', async () => {
  ...
  await assert.rejects(EncryptedVault.open(broken, password, true));
  assert.notEqual(storage.getItem(VAULT_KEY), null);
  await assert.rejects(EncryptedVault.open(storage, 'another valid password', true));
  const reopened = await EncryptedVault.open(storage, password, false);
