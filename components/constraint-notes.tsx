...
const notes: Record<Note, { reference: string; original?: string; explanation: string }> = {
  ...
  stress: { reference: '［4］SFC 8.2(n)注释(3)', original: '「定期进行压力测试」', explanation: '守则要求流动性压力测试。此处压力比例、5%现金参考目标及压力后额度公式是本工具的设置，不是SFC规定的固定数值，也不等同每日／每周流动资产要求。' },
...
export function ConstraintNotes({ items }: { items: Note[] }) {
  const { t } = useI18n();
  return <details className="space-y-2 border-t border-border/60 px-5 py-3 text-xs leading-5 text-muted-foreground">
    <summary className="cursor-pointer font-medium">{t("口径与限制依据")}</summary>
    <p><span className="font-medium">{t('测算范围')}</span><br />{t('仅校验已建模约束，不代表全面合规审查。')}</p>
    {items.map(item => <p key={item}><span className="font-medium">{t(notes[item].reference)}</span>{notes[item].original && <>{t("· 原文摘录：")}{t(notes[item].original!)}</>}<br />{t(notes[item].explanation)}</p>)}
  </details>;
}
