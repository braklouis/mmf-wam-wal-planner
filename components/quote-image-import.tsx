...
export function QuoteImageImport({ onText, t }: { onText: (text: string) => void; t: (text: string) => string }) {
  ...
  async function recognize() {
    ...
    running.current = true; setBusy(true); setError(''); setStatus('正在加载识别资源…');
    ...
    try {
      ...
      worker = await createWorker('eng+chi_sim+chi_tra', 1, { logger: message => {
      ...
      if (id === generation.current) setError(cause instanceof Error ? cause.message : '识别失败，请检查网络或换一张清晰截图。');
    } finally {
