...
export function QuoteImageImport({ onText, onWarnings, t }: { onText: (text: string) => void; onWarnings: (warnings: string[]) => void; t: (text: string) => string }) {
  ...
  async function recognize() {
    ...
    running.current = true; setBusy(true); setError(''); setStatus('1/4 · 正在加载识别资源…');
    ...
    try {
      ...
      worker = await createWorker(language, 1, { logger: message => {
        if (id === generation.current && phase === 'recognize' && message.status === 'recognizing text') {
          imageProgress = Math.max(imageProgress, Math.round(message.progress * 100));
          setStatus(`2/4 · 正在识别整张表格 ${imageProgress}%`);
        }
      } });
      if (id !== generation.current) return;
      workerRef.current = worker;
      const { words, warnings: reviewWarnings } = await recognizeQuoteImage(file, worker, () => id === generation.current, stage => {
        if (id !== generation.current) return;
        phase = stage.phase;
        if (phase === 'prepare') setStatus('2/4 · 正在增强表格文字…');
        else if (phase === 'recognize') setStatus('2/4 · 正在识别整张表格 0%');
        else if (phase === 'headers') setStatus(`3/4 · 核对期限表头：已完成 ${stage.completed}/${stage.total}`);
        else setStatus(`4/4 · 核对机构和报价：已完成 ${stage.completed}/${stage.total}`);
      }, language === 'eng');
      if (id !== generation.current) return;
      const text = ocrWordsToQuoteTable(words);
      onText(text); onWarnings(reviewWarnings);
