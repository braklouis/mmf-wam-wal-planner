export function parseQuoteTable(text: string) {
  ...
  if (!header || header[0].toLowerCase() !== 'bank' || header.slice(1).some(term => days[term] === undefined) || header.length < 2 || new Set(header).size !== header.length) throw new Error('请粘贴含 Bank 和期限表头的制表符分隔报价表。');
  const names = new Set<string>();
  const quotes: { bank: string; term: string; days: number; rate: number }[] = [];
  const cells = new Set<string>();
  for (const row of rows) {
    const bank = row[0];
    if (!bank || bank.toLowerCase() === 'max') continue;
    names.add(bank);
    for (let index = 1; index < row.length; index++) {
      if (!row[index]) continue;
      const term = header[index];
      if (!term || !/^-?\d+(?:\.\d+)?%?$/.test(row[index])) throw new Error(`${bank} ${term ?? ''}：报价格式无效。`);
      const rate = Number(row[index].replace('%', ''));
