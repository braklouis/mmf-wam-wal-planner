...
const englishCopy: Record<string, string> = {
  "暂时开放": "Temporarily available",
  "根据预测利率比较锁定与等待策略": "Compare locking rates with waiting under a forecast",
  "利率预测与等待策略": "Rate forecasts and waiting strategies",
  "统一比较期限（天）": "Comparison horizon (days)",
  "现金等待天数": "Cash waiting period (days)",
  "计息基准": "Day-count basis",
  "未来再投资利率曲线": "Forecast reinvestment curve",
  "粘贴预测曲线": "Paste forecast curve",
  "期限与预测利率": "Tenor and forecast rate",
  "将替换预测节点": "Forecast nodes to replace",
  "确认导入曲线": "Confirm curve import",
  "期限（天）": "Tenor (days)",
  "预测利率": "Forecast rate",
  "添加预测节点": "Add forecast node",
  "预测期限结构": "Forecast term structure",
  "比较预测策略": "Compare forecast strategies",
  "新增资金策略对比": "New-money strategy comparison",
  "现金等待": "Cash waiting",
  "预测策略未配置金额": "Unallocated in forecast strategy",
  "初始配置后 WAM / WAL": "Initial post-allocation WAM / WAL",
  "未参与比较的产品": "Products excluded from comparison",
  "当前报价策略预计收益": "Projected income: current-rate strategy",
  "预测策略预计收益": "Projected income: forecast strategy",
  "预计收益变化": "Projected income difference",
  "今日产品": "Current product",
  "当前报价": "Current rate",
  "首段天数": "Initial days",
  "再投资利率": "Reinvestment rate",
  "累计收益率": "Cumulative return",
  "当前报价配置": "Current-rate allocation",
  "预测策略配置": "Forecast allocation",
  "比较现在锁定、短期续投和现金等待。预测曲线用于到期后的再投资，不会覆盖今日报价。": "Compare locking rates now, short-term rollover and waiting in cash. The forecast applies to reinvestment and does not replace current quotes.",
  "横轴是再投资的产品期限，不是未来日期。每个初始产品到期时，都使用这条预测曲线。": "The horizontal axis is the reinvestment tenor, not a future date. The same forecast curve applies at every initial maturity.",
  "支持 Excel 粘贴或 CSV，可有表头。期限支持天数、1W、1M 等；1M 按 30 天换算。": "Paste Excel cells or CSV, with optional headers. Tenors accept days, 1W, 1M, etc.; 1M means 30 days.",
  "节点之间采用线性插值，不向区间外推算。缺少对应期限预测的产品会单独列为未参与。": "Rates are linearly interpolated between nodes, never extrapolated. Products without a covered reinvestment tenor are listed as excluded.",
  "计算方式：终值 = 本金 × (1 + 今日利率 × 首段天数 / 年基准) × (1 + 预测利率 × 剩余天数 / 年基准)。利率计算时除以 100。": "Terminal value = principal × (1 + current rate × initial days / year basis) × (1 + forecast rate × remaining days / year basis). Divide percentage rates by 100.",
  "现金等待使用今日可投中期限为 0 天的报价，等待期间利率假设不变；到期的本金和利息一起再投资。未配置资金按零收益计，不等同于现金等待。": "Cash waiting uses today’s zero-day quotes and assumes the cash rate stays constant during the wait. Principal and interest are reinvested together. Unallocated money earns zero and is distinct from cash waiting.",
  "只优化新增资金。初始配置沿用当前期限、额度和集中度约束；未来再投资假设可按预测利率成交，不验证未来额度、机构归属和逐日合规。浮息产品和比较日后才到期的产品暂不参与。": "Only new money is optimized. Current term, capacity and concentration constraints apply initially. Future reinvestment assumes the forecast rate is available; future capacity, institutions and daily compliance are not verified. Floating-rate products and maturities beyond the horizon are excluded.",
  "结果是该预测情景下的最优配置，不是保证收益；各产品需采用相同币种和计息口径，暂不计费用和提前卖出价格变化。": "The allocation is optimal under this forecast, not a guaranteed return. Use the same currency and interest convention across products. Fees and early-sale price changes are excluded.",
  "今日可投尚无 0 天现金报价，当前不会生成现金等待策略；请先添加现金利率及机构归属。": "No zero-day cash quote is available, so cash waiting is not included. Add a cash rate and institution in today’s products.",
  "两种配置使用相同可比较产品；收益均按这条预测曲线评估，输入改变后自动重算。当前报价基准以今日利率最大化，不代表最优择时。": "Both allocations use the same eligible products and are evaluated under this forecast. Changes recalculate automatically. The current-rate benchmark maximizes today’s yield, not investment timing.",
  "请补齐每个预测节点的期限和利率，或删除空行。": "Complete the tenor and rate of every forecast node, or delete empty rows.",
  "请提供期限和利率两列。": "Provide tenor and rate columns.",
  "请填写有效比较期限和计息基准。": "Enter a valid comparison horizon and day-count basis.",
  "只支持比较日内到期的定期产品。": "Only term products maturing within the horizon are supported.",
  "暂不支持浮息产品的重定价预测。": "Floating-rate repricing forecasts are not supported yet.",
  "当前报价格式无效。": "Invalid current quote.",
  "等待天数须大于零且不超过比较期限。": "Waiting days must be positive and no greater than the comparison horizon.",
  "利率或期限超出可计算范围。": "The rate or tenor is outside the supported numeric range.",
  "预测策略仅用于新增资金配置。": "Forecast strategies apply only to new-money allocation.",
  "请先在配置测算填写大于零的可配置金额。": "Enter a positive investable amount in the allocation planner first.",
  "没有可比较产品：请检查今日报价、比较期限和预测曲线覆盖范围。": "No comparable products. Check today’s quotes, the horizon and forecast curve coverage.",
  "预测收益超出可计算范围。": "Projected income is outside the supported numeric range.",
  "期限须为非负有限数值。": "Tenor must be a finite nonnegative number.",
  "请提供有效且期限不重复的利率节点。": "Provide valid rate nodes with unique tenors.",
  "目标期限不在输入区间内，不作外推。": "The target tenor is outside the curve; no extrapolation is applied.",
  "数值超出可计算范围。": "The value is outside the supported numeric range.",
  "锁定至比较日": "Lock until horizon",
  "短期续投": "Short-term rollover",
...
function toEnglish(input: string) {
  const curveError = input.match(/^第 (\d+) 行：期限或利率格式无效。$/);
  if (curveError) return `Row ${curveError[1]}: Invalid tenor or rate.`;
